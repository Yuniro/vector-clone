...
const Stats: FunctionComponent<Props> = ({ isDesktop }) => {
  ...
  return (
    <Box
      className={clsx(
        styles.dashboardStatContent,
        isDesktop && styles.dashboardStatContent__Desktop
      )}
    >
      {stats.map((st, idx) => (
        <Box
          key={`st_${idx}`}
          className={clsx(styles.dashboardStatContentItem)}
        >
          <Typography className={clsx(styles.dashboardStatContentItemTitle)}>
            {st.title}
          </Typography>
          <Typography className={clsx(styles.dashboardStatContentItemValue)}>
            {st.value}
          </Typography>
        </Box>
      ))}
