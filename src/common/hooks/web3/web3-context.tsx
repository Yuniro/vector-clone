...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
    ...
    new Web3Modal({
      ...
      providerOptions: {
        walletconnect: {
          ...
          options: {
            rpc: {
              [Networks.AVALANCHE_MAINNET]: uri,
            },
          },
        },
      },
  ...
  const _checkNetwork = (otherChainID: number): Boolean => {
    if (Number(otherChainID) !== Networks.AVALANCHE_MAINNET) {
      alert('Please switch your wallet to Avalanche network to use Piston!');
    }

    if (chainID !== otherChainID) {
      console.warn('You are switching networks: ', otherChainID);
      if (otherChainID === Networks.AVALANCHE_MAINNET) {
        setChainID(otherChainID);
        otherChainID === Networks.AVALANCHE_MAINNET ? setUri(getMainnetURI()) : setUri(getTestnetURI());
        return true;
      }
      return false;
    }
